		...
		<view class="cart-shell" v-else>
			...
			<view class="cart-list">
				<view class="cart" v-for="(item, index) in cart" :key="item.id">
					<view class="check-shell"><evan-checkbox class="check" v-model="item.checked" primary-color="red"></evan-checkbox></view>
					<view class="goods">
						<view class="img-shell"><image class="img" :src="item.image_url" mode="widthFix"></image></view>
						<view class="content">
							<view class="title">{{ item.title }}</view>
							<view class="spec-shell">
								<view class="spec" @tap="chooseSpec(index)">
									<view class="text">{{ item.spec.spec_name }}</view>
		...
		<tui-bottom-popup class="goods-info" :show="isShow" @close="isShow = false" height="750">
			<view class="popup-shell">
				...
				<scroll-view scroll-y="true" class="spec">
					...
					<view class="all">
						<text v-for="(item, s_index) in specArr" :key="item.id" :class="{ item: true, selected: s_index === index }" @tap="index = s_index">
							{{ item.spec_name }}
						</text>
					</view>
				</scroll-view>

				<view class="btn-shell"><button class="confirm-btn" @click="toggleSpec()">确认</button></view>
			</view>
		</tui-bottom-popup>

		<!-- 回到顶部 -->
		<view class="top-button" @click="ToTop" v-if="showtotop"><image src="../../static/images/index/icon/top.png" class="topimg"></image></view>
